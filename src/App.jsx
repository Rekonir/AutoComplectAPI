...
function App() {
  ...
  const addToCart = (priceId) => {
    const id = priceId;
    const url = `https://www.japarts.ru/?id=ws;action=addtobasket2;login=${login};pass=${password};priceid=${id};`;
    console.log(id);
    axios({
      method: "get",
      url: url,
          ...
          article.error === "NO RESULTS FOUND" ? (
            ...
            <div className="article" key={article.priceid}>
              <div className="makename">
                Производитель:{" "}
                {article.makename ? article.makename : "Undefined"}
              </div>
              <div className="detailname">
                Наименование:{" "}
                {article.detailname ? article.detailname : "Undefined"}
              </div>
              <div className="pricerur">
                Стоимость: {article.pricerur ? article.pricerur : "Undefined"} ₽
              </div>
              <div className="quantity">
                На складе осталось: {article.quantity ? article.quantity : "0"}{" "}
                шт.
              </div>
